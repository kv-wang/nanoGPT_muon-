...
def zeropower_via_newtonschulz5(G: Tensor) -> Tensor:
    ...
    '''
    推特上@YouJiacheng（Muon主要推动者之一）提出了一个非常机智的想法：每一步迭代我们可以使用不同的系数
    '''
...
class Muon(torch.optim.Optimizer):
    ...
    @torch.no_grad()
    def step(self):
        futures: list[torch.Future] = [] # torch.Future 是 PyTorch 分布式通信的异步结果对象
        ...
        '''
        dist.all_gather(...)：PyTorch 分布式通信操作，把每个进程（GPU）上的参数片段收集到所有进程。
        params_pad[base_i:base_i + self.world_size]：用于存放所有卡收集到的参数。
        params_pad[base_i + self.rank]：本进程（本卡）要发送的参数。
        async_op=True：异步执行，不会阻塞主程序。
        .get_future()：返回一个 torch.Future 对象，代表这个异步操作的“未来结果”。
        futures.append(...)：把这个 future 加入列表，后面可以统一等待所有通信完成。
        '''
        torch.futures.collect_all(futures).wait()
        '''
        返回一个新的 future，表示所有 futures 都完成时才算完成。
        .wait() 会阻塞，直到所有异步通信操作都完成。
        '''
