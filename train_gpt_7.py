...
if cli_args.wandb == "yes":
    if master_process:
        ...
        wandb.init(
            project="muon_nanogpt_NS_v7",  # <-- 修改为你的wandb项目名
...
if cli_args.muon_type == "default":
    optimizer2 = Muon(raw_model.transformer.h.parameters(), lr=0.1*args.learning_rate, momentum=cfg["default"],
                      rank=ddp_rank, world_size=ddp_world_size, type='default')
elif cli_args.muon_type == "double_momentum":
    optimizer2 = Muon(raw_model.transformer.h.parameters(), lr=0.1*args.learning_rate, momentum=cfg["double_momentum"],
                      rank=ddp_rank, world_size=ddp_world_size, type='double_momentum')
elif cli_args.muon_type == "svd_momentum":
    optimizer2 = Muon(raw_model.transformer.h.parameters(), lr=0.1*args.learning_rate, momentum=cfg["svd_momentum"],
                      rank=ddp_rank, world_size=ddp_world_size, type='svd_momentum')
elif cli_args.muon_type == "svd_momentum_v2":
    optimizer2 = Muon(raw_model.transformer.h.parameters(), lr=0.1*args.learning_rate, momentum=cfg["svd_momentum_v2"],
