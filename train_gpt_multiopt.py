...
parser.add_argument("--muon_type_1", type=str, default="default", help="Muon optimizer_1 type")
parser.add_argument("--muon_type_2", type=str, default="default", help="Muon optimizer_2 type")
parser.add_argument("--split", type = float, default=0.5, help="split of steps for two optimizers")
...
if cli_args.wandb == "yes":
    if master_process:
        ...
        print(logfile)
        wandb.init(
            project="muon_nanogpt_multiopt" + str([args.train_seq_len, args.val_seq_len]),  # <-- 修改为你的wandb项目名
...
class Muon(torch.optim.Optimizer):
    ...
    """
    def __init__(self, params, lr=0.02, momentum=(0.95,0.99), nesterov=True, ns_steps=5, rank=0, world_size=1, type='default'):
        self.rank = rank
            ...
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    if self.type == 'default':
                        buf.lerp_(g, 1 - group["momentum"][0])
                        g_muon = g.lerp_(buf, group["momentum"][0]) if group["nesterov"] else buf
                        ...
                        buf.copy_(buf_ortho.lerp_(g_ortho, 1 - group["momentum"][0]))
                        #buf.copy_(buf_ortho.mul_(group["momentum"]).add_(g_ortho))
                        if group["nesterov"]:
                            g_muon = g_ortho.lerp_(buf_ortho, group["momentum"][0]).flatten()
                        else:
                            g_muon = buf
                        # svd_momentum 不再额外正交化
                    elif self.type == 'svd_momentum_v2':
                        g_ortho = zeropower_via_newtonschulz5(g, steps=group["ns_steps"]).to(dtype=torch.float32)
                        buf.lerp_(g_ortho, 1 - group["momentum"][0])
                        g_muon = g_ortho.lerp_(buf, group["momentum"][0]).flatten().to(dtype=torch.bfloat16) if group["nesterov"] else buf.flatten()
...
optimizer_adam = torch.optim.Adam(adam_params, betas=(0.8, 0.95), eps=1e-10, fused=True)
optimizer1 = Muon(hidden_matrix_params, lr=0.05, momentum=cli_args.cfg_1, rank=rank, world_size=world_size, type=cli_args.muon_type_1)
optimizer2 = Muon(hidden_matrix_params, lr=0.05, momentum=cli_args.cfg_2, rank=rank, world_size=world_size, type=cli_args.muon_type_2)

optimizers = [optimizer_adam, optimizer1, optimizer2]
...
for step in tqdm(range(train_steps + 1)):
    ...
    if cli_args.muon_type == "default":
        ...
        for group in optimizer5.param_groups:
            frac = min(step / 300, 1)
            group["momentum"] = (1 - frac) * 0.85 + frac * cli_args.muon_momentum_0
    '''
    
    # step the optimizers
    optimizer_adam.step()  # Adam optimizer for head and embedding
    if step < train_steps * cli_args.split:
        optimizer1.step()
    else:
        optimizer2.step()
