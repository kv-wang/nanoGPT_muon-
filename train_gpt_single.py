...
parser.add_argument("--muon_type", type=str, default="default", choices=["default", "double_momentum", "svd_momentum"], help="Muon optimizer type")
...
if master_process:
    ...
    print(logfile)
    wandb.init(
        project="muon_nanogpt",  # <-- 修改为你的wandb项目名
        name=cli_args.muon_type,
        config=vars(args),
    )
            ...
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    if self.type == 'default':
                        ...
                        buf.copy_(buf_ortho.lerp_(g_ortho, 1 - group["momentum"]))
...
optimizer2 = Muon(hidden_matrix_params, lr=0.05, momentum=0.95, rank=rank, world_size=world_size, type="default")
optimizer3 = Muon(hidden_matrix_params, lr=0.05, momentum=[0.9, 0.95], rank=rank, world_size=world_size, type='double_momentum')
optimizer4 = Muon(hidden_matrix_params, lr=0.05, momentum=0.95, rank=rank, world_size=world_size, type='svd_momentum')
...
for step in tqdm(range(train_steps + 1)):
    ...
    if cli_args.muon_type == "default":
        ...
        for group in optimizer3.param_groups:
            frac = min(step / 300, 1) # momentum warmup for muon
            group["momentum"][0] = (1 - frac) * 0.80 + frac * 0.90
            group["momentum"][1] = (1 - frac) * 0.85 + frac * 0.95
